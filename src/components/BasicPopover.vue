  ...
  >
    ...
    >
      <PopoverPanel v-bind="panelAttributes">
        <div v-bind="innerPanelAttributes" :style="{ maxWidth: panelWidth + 'px' }">
...
const buttonOffsetFromCenter = computed(
  () =>
    buttonBoundingBox.left.value + buttonBoundingBox.width.value / 2 - windowSize.width.value / 2,
)
...
const panelOverflowDistance = computed(() => {
  if (buttonOffsetFromCenter.value >= 0) {
    ...
    const value =
      panelWidth.value / 2 - buttonBoundingBox.width.value / 2 - buttonBoundingBox.left.value
