  ...
  <div class="-mt-4 mb-12 flex flex-col gap-y-4 sm:gap-y-2">
    <SettingsNotification v-for="invite in invites" :key="invite.id">
      <SettingsNotificationText>
        ...
        <span class="lowercase"> {{ ' ' + invite.role }}. </span>
      </SettingsNotificationText>

      <template #buttons>
        <SettingsNotificationButton :separator="true" @click="reject(invite.id)">
          Reject
        </SettingsNotificationButton>
        <SettingsNotificationButton @click="accept(invite.id)"> Accept </SettingsNotificationButton>
      </template>
